...
```bash
sudo env HERMES_RDP_REF=v1.0.5 bash /tmp/install-hermes-rdp.sh --host SERVER_IP_OR_DOMAIN --telegram-token "$TG_TOKEN" --telegram-chat-id TELEGRAM_USER_ID
```
...
Версия исходников выбирается переменной окружения `HERMES_RDP_REF`. Для продакшена указывай релизный тег, например `v1.0.5`. Для тестовой ветки можно временно передать имя ветки.
