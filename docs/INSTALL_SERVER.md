...
Версия исходников выбирается переменной окружения `HERMES_RDP_REF`. Для продакшена указывай релизный тег, например `v1.0.7`. Для тестовой ветки можно временно передать имя ветки.
