...
The Win10 x64 + PowerShell x86 / WOW64 / Sysnative compatibility acceptance is **COMPLETE** for current product head `c51ed8...`. Do not repeat without a concrete regression reason.
...
## Current branch CI / merge gate

- PR #19: `fix/control-state-dashboard`.
- Accepted product head: `c51ed8fa2c090dbc731a0c06f357d899846e90ae`.
- CI #139 on that pre-reconciliation head: PASS, including Linux release checks and Windows PowerShell 5.1 parse/certificate pinning.
- All required live runtime acceptance for PR #19 is now **COMPLETE PASS**.
- `main` advanced through context-only commits after branch split; reconcile current `main` into the feature branch, rerun CI on the reconciled head, then recheck mergeability before merge.

## Remaining acceptance before merge

1. reconcile feature branch with current `main` without changing accepted product code;
2. verify reconciled diff/provenance;
3. rerun CI on reconciled head and recheck mergeability;
4. merge only when this repository gate is green.
